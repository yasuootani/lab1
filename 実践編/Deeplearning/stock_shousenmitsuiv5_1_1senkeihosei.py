...
def linear_interpolation(df, column_name):
    for i in range(1, len(df) - 1):
        if pd.isna(df.at[i, column_name]):
            start_idx = i - 1
            while start_idx >= 0 and pd.isna(df.at[start_idx, column_name]):
                start_idx -= 1
            end_idx = i + 1
            while end_idx < len(df) and pd.isna(df.at[end_idx, column_name]):
                end_idx += 1
            
            if start_idx >= 0 and end_idx < len(df):
                prev_value = df.at[start_idx, column_name]
                next_value = df.at[end_idx, column_name]
                interpolated_values = np.linspace(prev_value, next_value, end_idx - start_idx + 1)
                df.loc[start_idx+1:end_idx-1, column_name] = interpolated_values[1:-1]
    return df
...
column_name = 'Value'  # 列名を適切に指定


# データフレームにNaNが残っている場合、これを処理することができます
#data = data.ffill().bfill()

# データフレームに変更を保存することができます
...
# 線形補完を実行
data = linear_interpolation(merged_data, column_name)
data.to_csv('interpolated_data.csv', index=False)
# NaNの場合、直近の過去の値を使用
...
scaler = MinMaxScaler()
df_scaled = scaler.fit_transform(merged_data2[['close', 'Value']])
...
print(hyoka(df_test[3:,0],predicted_test_plot))