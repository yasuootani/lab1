...
keta = np.array([1000000] * 6, dtype=np.int32)
data_2003 = data_2003 / keta
data_2021 = data_2021 / keta
data_2003 = data_2003.astype(int)
data_2021 = data_2021.astype(int)
# グラフの初期化
...
trace_2003=go.Waterfall(
    ...
    name='2003増減',
    decreasing = dict(marker={'color': 'green'}),
    increasing = dict(marker={'color': 'lightblue'}),
    totals = dict(marker={'color': 'royalblue'}),
    connector = {'line': {'width': 1, 'color':'grey', 'dash': 'solid'}}
    
)

fig.add_trace(trace_2003,row=1,col=1)
...
trace_2021=go.Waterfall(
    ...
    measure=['absolute', 'relative', 'relative', 'relative', 'relative', 'total'],
    name='2021増減',
    decreasing = dict(marker={'color': 'orange'}),
    increasing = dict(marker={'color': 'pink'}),
    totals = dict(marker={'color': 'red'}),
    connector = {'line': {'width': 1, 'color':'blue', 'dash': 'solid'}}
)

fig.add_trace(trace_2021,row=2,col=1)
...
pyo.plot(fig, filename='C:\\Users\\ip2305\\Desktop\\python入門実践編\\jissenhen-sample\\実践編\\chapter05\\GDP\\g1.html', auto_open=True)
